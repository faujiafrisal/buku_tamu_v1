...
@section('content')
    <main
        class="w-full max-w-2xl gov-card rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl p-5 sm:p-7 relative overflow-hidden transition-all">
        ...
        <form id="guestForm" action="{{ route('guestbook.store') }}" method="POST" class="space-y-4">
            ...
            <div
                ...
                <div class="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-2">
                    <span
                        class="w-5 h-5 rounded-full bg-blue-800 text-white flex items-center justify-center text-[10px] font-bold">1</span>
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">Identitas
                        Pengunjung</h3>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="grid grid-cols-2 gap-3" id="jkGroup">
                        <button type="button" data-value="Laki-Laki"
                            class="jk-btn active flex items-center justify-center gap-2.5 py-3 px-4 rounded-xl border-2 border-blue-800 bg-blue-50 dark:bg-blue-950/80 text-blue-900 dark:text-blue-300 font-bold text-xs sm:text-sm transition-all shadow-sm cursor-pointer">
                            <i class="fa-solid fa-mars text-blue-600 dark:text-blue-400 text-base"></i> Laki-Laki
                        </button>
                        <button type="button" data-value="Perempuan"
                            class="jk-btn flex items-center justify-center gap-2.5 py-3 px-4 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium text-xs sm:text-sm transition-all shadow-sm cursor-pointer">
            ...
            <div
                ...
                <div class="flex items-center gap-2 border-b border-slate-200 dark:border-slate-800 pb-2">
                    <span
                        class="w-5 h-5 rounded-full bg-blue-800 text-white flex items-center justify-center text-[10px] font-bold">2</span>
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200">Detail
                        Kunjungan</h3>
                </div>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-1.5">
                            <span id="usiaBadge"
                                class="px-3 py-1 bg-blue-900 text-amber-400 font-extrabold text-xs rounded-lg border border-amber-500/40 shadow-sm">
                                20
                            </span>
                ...
                <div class="space-y-2 pt-1">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5" id="pillGroup">
                        <button type="button" data-value="1 orang"
                            class="pill-btn active flex items-center justify-center gap-2 py-3 px-3 rounded-xl border-2 border-blue-800 bg-blue-50 dark:bg-blue-950/80 text-blue-900 dark:text-blue-300 font-bold text-xs sm:text-sm transition-all shadow-sm cursor-pointer">
                            <i class="fa-solid fa-user text-xs"></i> 1 orang
                        </button>
                        <button type="button" data-value="2 orang"
                            class="pill-btn flex items-center justify-center gap-2 py-3 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium text-xs sm:text-sm transition-all shadow-sm cursor-pointer">
                            <i class="fa-solid fa-user-group text-xs"></i> 2 orang
                        </button>
                        <button type="button" data-value="3 orang"
                            class="pill-btn flex items-center justify-center gap-2 py-3 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium text-xs sm:text-sm transition-all shadow-sm cursor-pointer">
                            <i class="fa-solid fa-users text-xs"></i> 3 orang
                        </button>
                        <button type="button" data-value="4 orang"
                            class="pill-btn flex items-center justify-center gap-2 py-3 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium text-xs sm:text-sm transition-all shadow-sm cursor-pointer">
                            <i class="fa-solid fa-users-rectangle text-xs"></i> 4 orang
                        </button>
                        <button type="button" data-value="5-10 orang"
                            class="pill-btn col-span-2 sm:col-span-1 flex items-center justify-center gap-2 py-3 px-3 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-400 font-medium text-xs sm:text-sm transition-all shadow-sm cursor-pointer">
            ...
            <div
                ...
                <!-- Keperluan Kunjungan Field (Free Text / Textarea) -->
                <div class="space-y-1.5">
                    <label for="keperluanInput"
                        class="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
                        <span>Layanan & Keperluan <span class="text-rose-500">*</span></span>
                    </label>
                    <div class="relative">
                        <div class="absolute top-3 left-0 pl-3.5 flex items-start pointer-events-none text-slate-400">
                            <i class="fa-solid fa-clipboard-list text-sm"></i>
                        </div>
                        <textarea id="keperluanInput" name="keperluan" required rows="3"
                            placeholder="Tuliskan layanan atau keperluan Anda..."
            ...
            <div
                ...
                <div class="space-y-1.5">
                    <label for="kontakInput"
                        class="flex items-center justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
                        <span>Nomor WhatsApp Aktif <span class="text-rose-500">*</span></span>
                        <span
                            class="text-[10px] font-normal text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1">
            ...
            <div class="pt-2">
                <button type="submit" id="submitBtn"
                    ...
                    <i
                        class="fa-solid fa-paper-plane text-amber-400 group-hover:scale-110 transition-transform text-sm"></i>
                    <span>SIMPAN DATA KEHADIRAN</span>
                    <i
                        class="fa-solid fa-arrow-right text-xs text-amber-400 group-hover:translate-x-1 transition-transform"></i>
                </button>
            </div>

            <p class="text-center text-xs text-slate-500 dark:text-slate-400 pt-1 flex items-center justify-center gap-1.5">
                <i class="fa-solid fa-database text-emerald-600 dark:text-emerald-400"></i> Terintegrasi langsung dengan
                Database Laravel MPP Kota Samarinda
            </p>
        </form>
    </main>
    ...
    <div id="successModal"
        class="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-50 opacity-0 pointer-events-none transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-900 rounded-3xl p-6 sm:p-8 max-w-md w-full text-center shadow-2xl border-2 border-amber-500/40 transform scale-95 transition-transform duration-300"
            id="successCard">
            <div
                class="w-20 h-20 bg-gradient-to-tr from-emerald-600 to-teal-500 text-white rounded-3xl flex items-center justify-center mx-auto mb-4 text-3xl shadow-xl shadow-emerald-600/20 border-2 border-white">
                <i class="fa-solid fa-check-double"></i>
            </div>

            <span
                class="text-[10px] font-extrabold tracking-widest text-emerald-600 dark:text-emerald-400 uppercase bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">
            ...
            <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
                Data presensi Anda telah tersimpan secara resmi di Database Mal Pelayanan Publik (MPP) Kota Samarinda.
                Selamat melanjutkan pelayanan.
            </p>

            <button id="closeSuccessBtn"
                class="w-full py-4 bg-gradient-to-r from-blue-900 to-indigo-900 text-amber-400 font-extrabold rounded-xl shadow-lg border border-amber-500/40 transition-all active:scale-95 text-sm uppercase tracking-wide cursor-pointer">
                Kembali ke Formulir
            </button>
            <p id="countdownText"
                class="text-xs text-amber-600 dark:text-amber-400 font-semibold mt-3 flex items-center justify-center gap-1.5">
