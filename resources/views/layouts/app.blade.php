...
<body
    class="gov-pattern text-slate-800 dark:text-slate-100 font-sans min-h-screen transition-colors duration-300 flex flex-col justify-between items-center p-3 sm:p-6 md:p-8 selection:bg-blue-600 selection:text-white">
        ...
        <div class="flex items-center gap-2">
            @if(request()->is('admin'))
                <a href="{{ route('guestbook.index') }}"
                    class="px-3.5 py-2 rounded-xl bg-blue-900/80 hover:bg-blue-800 text-amber-400 border border-amber-500/30 text-xs font-bold transition-all shadow-sm flex items-center gap-1.5">
                    <i class="fa-solid fa-clipboard-user"></i> Formulir
                </a>
            @endif

            <button id="themeToggleBtn" title="Ganti Tema Visual"
                class="p-2.5 rounded-xl bg-slate-900/80 border border-slate-700 text-slate-200 hover:bg-slate-800 transition-all shadow-sm text-xs cursor-pointer">
    ...
    <script>
        function showToast(message, type = 'info') {
            ...
            toast.className = `toast-enter pointer-events-auto flex items-center gap-3 p-4 rounded-2xl shadow-xl text-xs font-semibold border ${type === 'error'
